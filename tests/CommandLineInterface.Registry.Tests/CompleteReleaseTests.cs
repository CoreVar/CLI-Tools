...
public sealed class CompleteReleaseTests : IDisposable
{
    ...
    public async Task ValidatesRidMatrixBeforePromotion()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => store.CompleteReleaseAsync("sample", "sample-cli", "1.0.0", ["linux-x64", "win-x64"], null, [], "stable", default).AsTask());
        ...
        await store.CompleteReleaseAsync("sample", "sample-cli", "1.0.0", ["linux-x64", "win-x64"], null, ["setup"], "stable", default);
