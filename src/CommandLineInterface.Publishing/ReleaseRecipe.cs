...
public sealed class ReleaseRecipe
{
    ...
    public Dictionary<string, string> Signatures { get; init; } = new(StringComparer.OrdinalIgnoreCase);
...
public sealed class ReleaseRecipePublisher(RegistryPublisher? publisher = null)
{
    ...
    public async ValueTask PublishAsync(ReleaseRecipe recipe, string? token = null, CancellationToken cancellationToken = default)
    {
        ...
        var signatures = new Dictionary<string, ArtifactSignature>();
        var trustedKeys = new Dictionary<string, string>();
        foreach (var artifact in recipe.Artifacts)
        {
            if (recipe.Signatures?.TryGetValue(artifact.Key, out var signatureFile) == true)
            {
                var signature = ArtifactSignature.Load(signatureFile);
                await signature.VerifyAsync(artifact.Value, cancellationToken);
                signatures.Add(artifact.Key, signature);
                if (trustedKeys.TryGetValue(signature.KeyId, out var existing) && existing != signature.PublicKeyPem)
                    throw new InvalidDataException("A signing key ID refers to multiple public keys.");
                trustedKeys[signature.KeyId] = signature.PublicKeyPem;
            }
            else if (recipe.RequireSignatures) throw new InvalidDataException($"A signature is required for '{artifact.Key}'.");
        }
        foreach (var artifact in recipe.Artifacts)
            await _publisher.PublishCliAsync(recipe.Endpoint, recipe.Tenant, recipe.Product, recipe.Version,
                artifact.Key, artifact.Value, uploadChannel, token, cancellationToken, signatures.GetValueOrDefault(artifact.Key));
        ...
        await File.WriteAllTextAsync(Path.Combine(installerOutput, "install.ps1"), InstallerScriptGenerator.PowerShell(recipe.Product, catalog, promoteChannel, trustedKeys, recipe.RequireSignatures), cancellationToken);
        await File.WriteAllTextAsync(Path.Combine(installerOutput, "install.sh"), InstallerScriptGenerator.Shell(recipe.Product, catalog, promoteChannel, trustedKeys, recipe.RequireSignatures), cancellationToken);
    ...
    public static ReleaseRecipe Load(string path)
    {
        ...
        foreach (var key in recipe.Signatures?.Keys.ToArray() ?? []) if (!Path.IsPathRooted(recipe.Signatures![key])) recipe.Signatures[key] = Path.GetFullPath(Path.Combine(root, recipe.Signatures[key]));
